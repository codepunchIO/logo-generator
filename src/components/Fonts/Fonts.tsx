...
const Fonts = () => {
  ...
  useEffect(() => {
    if (state.logo.data.colors) {
      const pallete_id = Number(state.logo.data.colors!) - 1;
        ...
        <div className="w-9/12 justify-center mx-auto">
          ...
          <div className="flex flex-row justify-evenly">
            {fontCategories.map((style, index) => (
              <div
                className="flex flex-col w-32 px-1 mx-5 h-12 bg-green-500 rounded-lg text-white justify-center w-48 px-1 text-center text-sm shadow-lg hover:shadow-2xl ease-in-out duration-200 cursor-pointer"
            ...
            <div className="flex flex-wrap flex-row justify-evenly p-4 h-full ">
              {fontLoaded ? (
                style.map((family: any, index) => (
                  <div
                    className={`${pallete} flex flex-row flex-nowrap hover:scale-100 my-2 duration-300 ease-in-out hover:shadow-2xl shadow-md rounded-lg h-40 w-72 justify-center`}
                ...
                <div className="flex mt-[200px] ">
                  <svg
                    role="status"
                    className="inline w-24 h-24 mr-2 text-gray-200 animate-spin dark:text-gray-600 fill-green-500"
                    viewBox="0 0 100 101"
                    fill="none"
